...
class UserFormStore extends NgSimpleStateBaseSignalStore<UserFormState> {
  ...
  selectIsFormValid(): Signal<boolean> {
    return this.selectState(
      (state) =>
        state.name.valid && state.email.valid && state.age.valid && state.acceptTerms.valid,
    );
  }
...
class TodoStore extends NgSimpleStateBaseSignalStore<TodoState> {
  ...
  selectCompletedCount(): Signal<number> {
    return this.selectState((state) => state.todos.filter((t) => t.completed).length);
  }
  ...
  addTodo(text: string, priority: 'low' | 'medium' | 'high' = 'medium'): boolean {
    return this.produce((draft) => {
      const newId = draft.todos.length > 0 ? Math.max(...draft.todos.map((t) => t.id)) + 1 : 1;
      draft.todos.push({
        id: newId,
        text,
        completed: false,
...
describe('Form State Management Integration Tests', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [provideNgSimpleState({ enableDevTool: false }), UserFormStore],
    });
...
describe('Produce Method Integration Tests', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [provideNgSimpleState({ enableDevTool: false }), TodoStore],
    });
...
describe('LinkedState Integration Tests', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [provideNgSimpleState({ enableDevTool: false }), CounterWithHistoryStore],
    });
...
describe('Effects Integration Tests', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [provideNgSimpleState({ enableDevTool: false }), NotificationStore],
    });

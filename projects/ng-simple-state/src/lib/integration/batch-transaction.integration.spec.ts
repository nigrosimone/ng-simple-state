...
describe('Batch Operations Integration Tests', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [provideNgSimpleState({ enableDevTool: false }), OrderStore],
    });
  ...
  describe('Transaction Operations', () => {
    ...
    it('should maintain transaction isolation', async () => {
      ...
      expect(results).toEqual(['active: true', 'items after add: 1', 'active after commit: false']);
    });
  });
  ...
  describe('Debounced Updates', () => {
    ...
    it('should debounce rapid updates', () => {
      ...
      const { update } = createDebouncedUpdater<OrderState>((state) => updates.push(state), 200);
    ...
    it('should merge debounced updates', () => {
      ...
      const { update } = createDebouncedUpdater<OrderState>((state) => updates.push(state), 100);
  ...
  describe('Throttled Updates', () => {
    ...
    it('should execute first update immediately', () => {
      ...
      const { update } = createThrottledUpdater<OrderState>((state) => updates.push(state), 100);
